...
Next, a two-stage high-energy extraction ensures consistent and content-rich inputs. A 1.5-second sliding window (stride 0.2~s) first identifies the segment with the highest total energy, followed by a finer 1-second search (stride 20~ms) within that region to locate the most informative sub-segment. This hierarchical approach captures the most relevant portion of each utterance while standardizing duration.
...
\subsection*{Preprocessing Ablation Protocol}

To directly quantify the contribution of each preprocessing component, we define a step-wise ablation protocol based on the implemented pipeline in our public repository. All ablation variants share the same downstream feature extraction and model training configuration, so performance differences can be attributed to preprocessing choices rather than model or optimization changes.

The common front-end constraints are fixed as follows: mono conversion, resampling to 16~kHz, and extraction of a fixed 1-second segment via high-energy selection. The full preprocessing pipeline consists of three switchable components:
\begin{itemize}
\item \textbf{Denoising}: spectral noise reduction using the \texttt{noisereduce} implementation.
\item \textbf{WebRTC VAD}: frame-level speech masking with 30~ms frames and sensitivity level~2.
\item \textbf{Peak normalization}: waveform amplitude scaling to 0.99 of full range before 16-bit conversion.
\end{itemize}

We evaluate eight configurations: (i) no optional preprocessing, (ii) denoising only, (iii) VAD only, (iv) normalization only, (v) denoising+VAD, (vi) denoising+normalization, (vii) VAD+normalization, and (viii) denoising+VAD+normalization (full pipeline). For each configuration, Mel-spectrograms are extracted with the same parameters ($n\_fft=2048$, $hop\_length=128$, $n\_{mels}=128$, $f_{min}=20$~Hz), converted to log scale, Min--Max normalized, and resized to $128\\times32$.

This ablation design is used to report test Accuracy, Macro-F1, and Weighted-F1 under the same split and seed policy as the main experiment.
...
The dataset of 4,460 Vietnamese speech command samples was divided using a stratified split to maintain class balance across subsets: 75\% for training (3,341 samples), 15\% for validation (665 samples), and 10\% for testing (454 samples). Each subset preserves proportional representation of all 13 classes. This split policy was fixed and shared by all compared architectures.
...
To address fairness concerns in cross-model evaluation, we define a controlled benchmark in which ConvMixer-256/8, ResNet-18, MobileNetV2, EfficientNet, and AST (tiny configuration) are trained on the same dataset split, with aligned preprocessing, Mel-spectrogram extraction settings, augmentation strategy, optimizer family, early stopping policy, and epoch budget. This design aims to isolate architectural effects and minimize confounding factors caused by inconsistent pipelines.
...
\centering
\small
\caption{\label{tab:benchmark_protocol_summary}Controlled benchmark protocol summary for cross-model comparison.}
\begin{tabular}{p{0.34\linewidth}p{0.60\linewidth}}
\hline
\textbf{Protocol Item} & \textbf{Setting} \\
\hline
Compared models & ConvMixer-256/8, ResNet-18, MobileNetV2, EfficientNet-B0, AST (tiny configuration) \\
Dataset and classes & Vietnamese speech commands, 13 classes, 4{,}460 samples \\
Split policy & Stratified split with fixed ratios: 75\% train, 15\% validation, 10\% test \\
Split reproducibility & Fixed split seed and identical split for all models \\
Audio preprocessing & Shared denoising, WebRTC VAD, high-energy segment extraction, peak normalization \\
Feature representation & Mel-spectrogram, $n\_fft=2048$, $hop\_length=128$, $n\_{mels}=128$, resized to $128\\times32$ \\
Augmentation policy & Shared Time Masking, Frequency Masking, and Time Stretching configuration \\
Optimization family & Adam-based optimization with aligned scheduling and early-stopping rules \\
Training budget & Shared epoch budget and stopping criteria across models \\
Evaluation metrics & Accuracy, Macro-F1, Weighted-F1, test loss, parameter count, and runtime \\
Statistical reporting & Five seeds (42--46), reported as mean$\pm$std \\
\hline
\end{tabular}
\normalsize
\end{table}
...
\]
This protocol reduces dependence on a single random run and provides a more stable basis for model comparison. For reference, the current ConvMixer-256/8 run reports 97.14\% test accuracy, macro-F1 0.9716, weighted-F1 0.9714, and test loss 0.1103.
