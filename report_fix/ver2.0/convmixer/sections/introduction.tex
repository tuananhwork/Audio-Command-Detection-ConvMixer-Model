...
Formally, this work addresses the following problem: given short Vietnamese utterances recorded in practical smart-home conditions, can a compact patch-based convolutional model achieve strong and stable command classification performance under a controlled training protocol, while remaining suitable for edge-oriented deployment constraints (model size, training/inference cost, and reproducibility across seeds)?
To answer this question, we explicitly optimize for a three-way trade-off among recognition quality, computational efficiency, and statistical reliability, rather than maximizing single-run accuracy alone.

To the best of our knowledge, this study is an early systematic effort to extend the ConvMixer architecture from computer vision to the audio domain.
...
Second, we implement and train a compact ConvMixer model (8 blocks, 256 channels, and approximately 0.72 million parameters, i.e., 719,117 trainable parameters) on the 13-class Vietnamese speech command dataset with targeted data augmentation.
Third, we establish a controlled comparison protocol on the same dataset and training budget for ConvMixer-256/8, ResNet-18, MobileNetV2, EfficientNet, and AST (tiny configuration), enabling a fairer model-level assessment under shared preprocessing and feature settings.
Fourth, we strengthen statistical reliability by evaluating all compared models over five random seeds (42--46) and reporting mean$\pm$std metrics, while retaining detailed class-wise error analysis to explain model behavior; the current ConvMixer-256/8 reference figures are 97.14\% accuracy, macro-F1 0.9716, and weighted-F1 0.9714.
